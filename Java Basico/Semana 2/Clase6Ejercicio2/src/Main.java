...
g) Mostrar siempre por consola un mensaje informativo luego de resolver cada punto.*/
    ...
    public static void main(String[] args) {
        ...
        System.out.println("El producto con más stock es " + listaProductos.get(id).getNombre() + " con " + listaProductos.get(id).getCantStock() + " unidades de stock");

        //Restamos 3 unidades seteando el producto con mayor stock
        listaProductos.get(id).setCantStock(listaProductos.get(id).getCantStock() - 3);

        System.out.println("Ahora al producto " + listaProductos.get(id).getNombre() + " le restamos 3 unidades y pasa a tener " + listaProductos.get(id).getCantStock() + " unidades de stock");
